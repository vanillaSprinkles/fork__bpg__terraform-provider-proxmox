...
fake_ticket_3=$fake_ticket
fake_csrf_3=$fake_csrf
...
#### nested for-loop
## first loop over "real" and "fake" as variable name prefixes
...
for loop_real_fake in "real" "fake" ; do
    ...
    loop_rf_test_msg_fmt="${loop_real_fake}_test_msg_fmt"
    loop_rf_test_msg_fmt="${!loop_rf_test_msg_fmt}"
	...
	test_cmd ${loop_rf_pass_fail} ${i} 1
	...
	if [[ -n "${apitok}" ]] && [[ -z "${un}" && -z "${PROXMOX_VE_AUTH_TICKET}" ]]; then
	    ...
	    test_pre_auth_inline_empty_creds=1
	    curl_api_unlock_user_tfa "${un}"
